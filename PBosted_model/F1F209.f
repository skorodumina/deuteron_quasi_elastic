...
c      for a proton with Fermi motion of a deuteron
...
c------Added by Iu. Skorodumina-----
c subroutine elas. Returns the e-p differential cross section
c     as a function of the electron scattering angle theta.
c 
c subroutine elasrad. Returns radiated cross section using equation II.6 of
...
c      For Gleb & Iuliia Test purposes
      F1 = pm * W1 * emcfac 
      F2 = nu * W2 * emcfac 
c       F1 = pm * W1 
c       F2 = nu * W2 
...
c    nicely with Osipenko et al data from CLAS, up to 5 GeV**2
      ...
      real*8 qv, TAU, W1, W2, FY,FY1, dwmin, w2p
...
c-----------------------------------------------------------------
      ...
      subroutine elas(eb,theta,crsect_elast)
...
c     theta: scattering angle (deg)
      ...
      real*4 theta,q                !electron scattering angle
      real*4 rm_p			!proton mass
      real*4 s2,c2,theta2
      real*4 crsect_elast,rmott            !elastic cross-dection radiative corrected
      
      
      data rm_p/.938/
      
c      print *,'eb = ',eb,'theta = ',theta
...
c-----------------------------------------------  
      ...
      subroutine elasrad( es, theta_d, t, wcut, crsect_elast_rad,corrfact )  
...
c     This program calls function elas defined above.
      ...
      real*8 arg
      real*4 theta_d,t,wcut
      real*4 crsect_elast_rad,corrfact
      real*4 me,mp,pi,alpha,z,theta,delta,spence,es
...
c       print *,'es = ',es,'theta_d = ',theta_d, 't = ', t, 'wcut = ', wcut
c      write(6,*) 'Hello' 
c      write(6,*) 'Received ', es, theta_d, t, wcut
     ...
     * alog((e4+mp)/2/mp))
      ...
      CALL elas(es,theta_d,sigunp)
c      sigunp  = elas(es,theta_d)
c      print *,'sigunp = ', sigunp
...
c      elasrad = radcor*sigunp
      crsect_elast_rad = radcor*sigunp
      corrfact = radcor
      end
